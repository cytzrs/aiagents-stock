...
from lht_analysis import LhtAnalyzer
from lht_pdf_generator import display_report_download_section
from lht_history_ui import display_batch_history
import pandas as pd

def display_lht_selector():
    """显示龙回头选股界面"""
    
    # 检查是否触发批量分析（不立即删除标志）
    if st.session_state.get('lht_batch_trigger'):
        run_lht_batch_analysis()
        return
    
    # 检查是否查看历史记录
    if st.session_state.get('lht_view_history'):
    ...
    **筛选标准**:
    ...
    # 开始分析按钮
    if st.button("🚀 开始龙回头选股", type="primary", use_container_width=True):
        
        with st.spinner("正在获取数据并分析，这可能需要几分钟..."):
            
            # 创建分析器
            analyzer = LhtAnalyzer(model=model)
...
def display_analysis_results(result: dict, analyzer):
    ...
    if analyzer and analyzer.raw_stocks is not None and not analyzer.raw_stocks.empty:
        ...
        with col_batch1:
            st.markdown("#### 🚀 批量深度分析")
            st.caption("对15日内三次以上涨停，最近3个交易日回撤超过15%的TOP股票进行完整的AI团队分析，获取投资评级和关键价位")
...
def run_lht_batch_analysis():
    """执行龙回头选股TOP股票批量分析（遵循统一调用规范）"""
    import time
    import re
    
    st.markdown("## 🚀 龙回头选股TOP股票批量分析")
    st.markdown("---")
    
    # 检查是否已有分析结果
    if st.session_state.get('lht_batch_results'):
        display_lht_batch_results(st.session_state.lht_batch_results)
        ...
        with col_back:
            if st.button("🔙 返回主力选股", use_container_width=True):
                # 清除所有批量分析相关状态
                if 'lht_batch_trigger' in st.session_state:
                    del st.session_state.lht_batch_trigger
                if 'lht_batch_codes' in st.session_state:
                    del st.session_state.lht_batch_codes
                if 'lht_batch_results' in st.session_state:
                    del st.session_state.lht_batch_results
                st.rerun()
    ...
    # 获取股票代码列表
    stock_codes = st.session_state.get('lht_batch_codes', [])
    ...
    if start_analysis:
        ...
        # 保存结果到session_state
        st.session_state.lhte_batch_results = {
            "results": results,
            "total": len(results),
...
def display_lht_batch_results(batch_results):
    """显示主力选股批量分析结果"""
    import re
