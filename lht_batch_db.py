...
class MainForceBatchDatabase:
    ...
    def _init_database(self):
        ...
        # 批量分析历史记录表
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS batch_lht_analysis_history (
        ...
        cursor.execute('''
            ...
            ON batch_lht_analysis_history(analysis_date)
        ''')
    ...
    def save_batch_analysis(
        ...
        cursor.execute('''
            INSERT INTO batch_lht_analysis_history 
    ...
    def delete_record(self, record_id: int) -> bool:
        ...
        cursor.execute('DELETE FROM batch_lht_analysis_history WHERE id = ?', (record_id,))
    ...
    def get_statistics(self) -> Dict:
        ...
        # 总记录数
        cursor.execute('SELECT COUNT(*) FROM batch_lht_analysis_history')
        total_records = cursor.fetchone()[0]
        
        # 总分析股票数
        cursor.execute('SELECT SUM(batch_count) FROM batch_lht_analysis_history')
        total_stocks = cursor.fetchone()[0] or 0
        
        # 总成功数
        cursor.execute('SELECT SUM(success_count) FROM batch_lht_analysis_history')
        total_success = cursor.fetchone()[0] or 0
        
        # 总失败数
        cursor.execute('SELECT SUM(failed_count) FROM batch_lht_analysis_history')
        total_failed = cursor.fetchone()[0] or 0
        
        # 平均耗时
        cursor.execute('SELECT AVG(total_time) FROM batch_lht_analysis_history')
